...
## Git hygiene — commits get squashed regularly

This repo's history is kept tidy by squashing on a regular basis: a day's
worth of work-in-progress commits gets collapsed into a single, well-described
commit. Don't rely on individual intraday commit hashes staying around, and
expect history to be rewritten (and force-pushed on feature branches) when a
batch of commits is squashed down.
