...
2. **Discord `#tracker-download`** (the one-file installer for friends).
   - The attachment is `h3-tracker.bat`, generated by
     `python packaging/make-installer.py <out>` (installer-template.bat +
     `friends.env` embedded as `::ENV::` lines — contains secrets, NEVER commit
     the generated file; write it to the scratchpad). The bat installs to
     `%USERPROFILE%\h3-tracker` from the GitHub `releases/latest` zip and
     self-updates on every launch, so it usually only needs re-uploading when
     the embedded `.env` or the installer logic changes — not per release.
